...
make_anim_method('cam_punch', function(self)
    local entry = self:_then(_move {cam, Vector3(0, -0.2, 0), RELATIVE}):_with(parse_easing({'shake', 20, 0.5})):_for(0.4
        ):_and(Anita.CameraSizeAnimationProperty(cam, 0.9, RELATIVE)):_with(parse_easing({1, 0})):_for(0.05
        ):_then(Anita.CameraSizeAnimationProperty(cam, 1 / 0.9, RELATIVE)):_with(parse_easing()):_for(0.35)
    entry.head = self
    return entry
end)
