...
function label(name, display_name)
    ...
    if __Anita.scriptLoader.stateLocale == Anita.I18n.DefaultLocale then
        __Anita.scriptLoader:RegisterNewNode(name)
    else
        __Anita.scriptLoader:BeginAddLocaleForNode(name)
    end

    Anita.I18nHelper.NodeNames:Set(name, __Anita.scriptLoader.stateLocale, display_name)
end

--- jump to the given destination
--- should be called at the end of the node
function jump_to(dest)
    __Anita.scriptLoader:RegisterJump(dest)
end
...
function branch(branches)
    for i, branch in ipairs(branches) do
        ...
        if branch.image then
            ...
            image_info = Anita.BranchImageInformation(image_name, pos_x, pos_y, scale)
        end

        local mode = Anita.BranchMode.Normal
        if branch.mode == nil or branch.mode == 'normal' then
            -- pass
        elseif branch.mode == 'jump' then
            mode = Anita.BranchMode.Jump
        elseif branch.mode == 'show' then
            mode = Anita.BranchMode.Show
        elseif branch.mode == 'enable' then
            mode = Anita.BranchMode.Enable
        else
        ...
        __Anita.scriptLoader:RegisterBranch(tostring(i), branch.dest, branch.text, image_info, mode, cond)
    end
    __Anita.scriptLoader:EndRegisterBranch()
end
...
function is_start(name)
    __Anita.scriptLoader:SetCurrentAsStart(name)
end
...
function is_unlocked_start(name)
    __Anita.scriptLoader:SetCurrentAsUnlockedStart(name)
end
...
function is_default_start(name)
    __Anita.scriptLoader:SetCurrentAsDefaultStart(name)
end
...
function is_end(name)
    __Anita.scriptLoader:SetCurrentAsEnd(name)
end
