local shader_alias_map = {
    ...
    -- chenge_texture_shade = 'Chenge Texture With Fade',
}
...
local function get_full_shader_name(shader_name, pp)
    ...
    if pp then
        full_shader_name = 'Anita/Post Processing/' .. base_shader_name
    else
        full_shader_name = 'Anita/VFX/' .. base_shader_name
    end
...
local function get_mat(obj, shader_name, restorable)
    ...
    local pp = obj:GetComponent(typeof(Anita.PostProcessing))
    ...
    local pool = Anita.MaterialPool.Ensure(get_go(obj))
    local mat
    if restorable then
...
local function get_default_mat(obj)
    return Anita.MaterialPool.Ensure(get_go(obj)).defaultMaterial
end
...
local function set_mat_properties(mat, base_shader_name, properties)
    ...
    for name, value in pairs(properties) do
        ...
        elseif dtype == '2D' then
            if tostring(mat:GetType()) == 'Anita.RestorableMaterial' then
                mat:SetTexturePath(name, value)
            else
                local tex
                -- we cannot set value = nil in table, so we use ''
                if value and value ~= '' then
                    tex = Anita.AssetLoader.LoadTexture(value)
                end
...
local function parse_times(times)
    if times == nil then
        return 1, Anita.AnimationEntry.LinearEasing()
    elseif type(times) == 'number' then
        return times, Anita.AnimationEntry.LinearEasing()
    else
        -- type(times) == 'table'
...
make_anim_method('trans', function(self, obj, image_name, shader_layer, times, properties, color2)
    ...
    if tostring(obj:GetType()) == 'Anita.CameraController' then
        action_begin = function()
            __Anita.screenCapturer:CaptureGameTexture()
            mat:SetTexture('_SubTex', __Anita.screenCapturer.capturedGameTexture)
        ...
        action_end = function()
            ...
            __Anita.screenCapturer:DestroyGameTexture()
        end
    else
        action_begin = function()
            if obj.currentImageName and obj.currentImageName ~= '' then
                local tex = Anita.AssetLoader.LoadTexture(obj.imageFolder .. '/' .. obj.currentImageName)
...
function vfx(obj, shader_layer, t, properties)
    ...
    if shader_name then
        ...
        if tostring(obj:GetType()) == 'Anita.CameraOverlayMask' then
