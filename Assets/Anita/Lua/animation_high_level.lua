...
function get_coord(obj)
    ...
    local y = _pos.y
    local scale
    local camera = obj:GetComponent(typeof(Anita.CameraController))
    if camera then
        scale = camera.size
    else
...
local easing_func_name_map = {
    bezier = Anita.AnimationEntry.BezierEasing,
    cubic = Anita.AnimationEntry.CubicEasing,
    linear = Anita.AnimationEntry.LinearEasing,
    shake = Anita.AnimationEntry.ShakeEasing,
    shake_sqr = Anita.AnimationEntry.ShakeSquaredEasing,
}
...
function parse_easing(t)
    ...
    if t == nil then
        easing = Anita.AnimationEntry.CubicEasing(0, 0)
    ...
    if easing == nil then
        if start_slope == 1 and target_slope == 1 then
            easing = Anita.AnimationEntry.LinearEasing()
        else
            easing = Anita.AnimationEntry.CubicEasing(start_slope, target_slope)
        end
    end
...
make_anim_method('move', function(self, obj, coord, duration, easing)
    ...
    if camera then
        if relative then
            entry = self:_then(Anita.CameraSizeAnimationProperty(camera, scale, relative)):_with(easing):_for(duration)
        else
            entry = self:_then(Anita.CameraSizeAnimationProperty(camera, scale)):_with(easing):_for(duration)
        end
    else
...
function get_color(obj)
    local renderer = obj:GetComponent(typeof(UnityEngine.SpriteRenderer)) or obj:GetComponent(typeof(Anita.CharacterController)) or obj:GetComponent(typeof(UnityEngine.UI.Image))
    if renderer then
...
make_anim_method('tint', function(self, obj, color, duration, easing)
    local character = obj:GetComponent(typeof(Anita.CharacterController))
    ...
    local property
    if character then
        property = Anita.ColorAnimationProperty(Anita.CharacterColor(character, Anita.CharacterColor.Type.Base), parse_color(color))
    else
        property = Anita.ColorAnimationProperty(renderer, parse_color(color))
    end
...
make_anim_method('env_tint', function(self, obj, color, duration, easing)
    ...
    duration = duration or 1
    easing = parse_easing(easing)
    local property = Anita.ColorAnimationProperty(Anita.CharacterColor(character, Anita.CharacterColor.Type.Environment), parse_color(color))
    return self:_then(property):_with(easing):_for(duration)
end)
