function sound(audio_name, volume, pos, use_3d)
    ...
    pos = pos or {0, 0, 0}
    if use_3d then
        __Anita.soundController:PlayClipAtPoint(audio_name, Vector3(unpack(pos)), volume)
    else
        __Anita.soundController:PlayClipNo3D(audio_name, Vector3(unpack(pos)), volume)
    end
end
...
function play(obj, audio_name, volume)
    ...
    if tostring(obj:GetType()) == 'Anita.AudioController' then
        __Anita.musicUnlockHelper:Unlock(obj.audioFolder .. '/' .. audio_name)
    end
end
...
make_anim_method('volume', function(self, obj, value, duration)
    ...
    return self:_then(Anita.VolumeAnimationProperty(obj, value)):_for(duration)
end)
