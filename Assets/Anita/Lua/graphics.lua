...
function show(obj, image_name, coord, color, fade)
    ...
    if tostring(obj:GetType()) == 'Anita.SpriteController' then
        __Anita.imageUnlockHelper:Unlock(obj.imageFolder .. '/' .. image_name)
    end
end
...
function set_render_queue(obj, to)
    Anita.RenderQueueOverrider.Ensure(get_go(obj)).renderQueue = to
end
