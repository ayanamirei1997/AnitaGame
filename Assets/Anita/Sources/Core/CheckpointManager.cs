    ...
    public class Bookmark
    {
        ...
        public Texture2D screenshot
        {
            get
            {
                ...
                {
                    Assert.IsTrue(screenshotTexture == null, "Anita: Screenshot cache is not consistent.");
                    return null;
                }
    ...
    public class CheckpointManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (File.Exists(globalSavePath))
            {
                ...
                {
                    Debug.LogErrorFormat("Anita: Cannot load global save file: {0}", ex.Message);
                    Alert.Show(
                        null,
            ...
            // Debug.Log("Anita: CheckpointManager initialized.");
        }
        ...
        private T SafeRead<T>(string path, Action<T> assertion)
        {
            try
            {
                ...
                {
                    Debug.LogWarning($"Anita: {path} is corrupted, details below. Try to recover");
            ...
            {
                Debug.LogError($"Anita: Error loading {path}, details below");
        ...
        private T ReadSave<T>(Stream s)
        {
            using (var bw = new BinaryReader(s))
            {
                ...
                this.RuntimeAssert(Version >= version,
                    "Save file is incompatible with the current version of Anita.");
